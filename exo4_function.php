...
function checkAnswerString($ans, $gLeft){
    $numAlpha = 26;
    foreach (range('a', 'z') as $letter) {
        if (str_contains($ans, $letter)){
            $numAlpha --;
        }
    }
    if ($numAlpha <= $gLeft){
        echo "The current answer string includes too many alphabetic characters. A win is mathematically assured, with ".$numAlpha." wrong guesses.";
    }
}


function checkGuessesLeft($mys, &$gLeft) {
    if ($gLeft <= 0){
...
function checkForEmptyString($mys, $char, $gLeft){
    if ($char == ""){
...
function checkIfAlreadyGuessed($alphaP, $char, $ans, $mys, $gLeft){
    if ($alphaP[$char] !== "unguessed" && $ans != $mys){
        echo "<div class='board'>".$mys."</div>";
...
function checkChar($char, $ans, $mys, &$gLeft, $alphaP){
    ...
    if ($gLeft > 0 && $char != "" && $alphaP[$char] == "unguessed" && $ans != $mys){
        $goodGuessFlag = false;
        for($i = 0; $i < strlen($ans); $i++){
...
/*
    ...
    //the system accounts for entries with no value (like refreshing the browser)

    echo "Submit an empty string (which could happen on refresh).";
    $character = "";
    checkChar($character, $answerStr, $mysteryStr, $guessesLeft, $alphabetPool);



    //the system accounts for double entries (re-entering a character)

    echo "Re-enter a character that was already entered(a).";
    $character = "a";
    checkChar($character, $answerStr, $mysteryStr, $guessesLeft, $alphabetPool);
    checkChar($character, $answerStr, $mysteryStr, $guessesLeft, $alphabetPool);
    ...
    //The system accounts for caps, and ensures that the allowable inputs
    //(lowercase characters) will reveal the associated character as it appears
    //in the Answer string (even if it is uppercase).
    
    echo "Submit a lowercase d, when there is an uppercase D in the answer.";
    $character = "d";
    checkChar($character, $answerStr, $mysteryStr, $guessesLeft, $alphabetPool);



    //Once the player's guesses have elapsed, futher guesses will not be accepted

    echo "Once the player's guesses have elapsed, futher guesses will not be accepted";
    $character = "m";
    $guessesLeft = 0;
    checkChar($character, $answerStr, $mysteryStr, $guessesLeft, $alphabetPool);
*/
